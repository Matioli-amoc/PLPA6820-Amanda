...
STAND*T**r**e**a**t**m**e**n**t* &lt; −*a**s*.*f**a**c**t**o**r*(*S**T**A**N**D*Treatment)
\# example shown here.
...
<!-- -->
    ...
    ## 
    ## Call:
    ## lm(formula = Emergence ~ Treatment * DaysAfterPlanting, data = PlantE)
    ## 
    ## Residuals:
...
A- No, we dont need the interaction term, because p value is &gt; than
0.05.
...
A- The intercept represents the mean for Treatment 1 at the first day.
When we look at the results for Treatment 2, we can see that, on
average, the treatment 2 results in 134.53 fewer comparing with
treatment 1. That shows us that the difference between treatments is
significant (p&lt;0.001)
...
![](Code-challenging-7_files/figure-markdown_strict/unnamed-chunk-7-1.png)

A- The bars are representing the mean of the merged plans for each
treatment, and the error bars shows the standard error. The letters
shows the significant differences, for the Tukey test at alpha = 0.05.
The treatments that share the same letter are not significantly
different, while the treatments that shows different letters are
significantly different. For example: The treatment 2 has significantly
fewer emerged plants than most of the other treatments, while the
treatments 1,3,4,5,6,8 and 9 do not show any significant difference
among each other. The treatment 7, the letter “b” shows that this
treatment is significantly lower than the treatments labeled “a” (3, 5
and 6) but does not differ form treatments labeled “ab”
